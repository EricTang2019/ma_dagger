"""
Submit the ma_dagger job (Azure TRAPI teacher) without using the notebook.

Defaults mirror the previous notebook:
- VC: msrresrchbasicvc / Singularity.ND96_H100_v5 (Basic/High)
- Dataset: aimo, batch 8
- Model: Qwen/Qwen3-4B for gen/ver
- Teacher: TRAPI gpt-5_2025-08-07 @ gcr/shared, scope api://trapi, api-version 2025-02-01-preview

AZURE_OPENAI_AD_TOKEN is honored if provided (e.g., from `az account get-access-token --resource api://trapi`).
Otherwise build_job_command will fetch via MI/CLI.
"""
...
def main() -> None:
    ...
    # Tokens: prefer env, fallback to ~/.bashrc for convenience.
    ...
    azure_token = os.environ.get("AZURE_OPENAI_AD_TOKEN")  # optional
    ...
    # Submit one job per VC in the list.
    ...
    for idx, (vc, instance_type, sla_tier, priority, gpu_num) in enumerate(candidate_vcs):
        this_runid = (
            ...
            f"{vc}-{instance_type}-shard{idx:02d}"
        ).replace(".", "-")
        ...
        env_vars = {
            ...
            "TRIAPI_SCOPE": "api://trapi",
            "AZURE_API_VERSION": "2025-02-01-preview",
        }
        if azure_token:
            env_vars["AZURE_OPENAI_AD_TOKEN"] = azure_token
